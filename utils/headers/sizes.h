...
long calc_offset_dis(uint dx, gap_buf &buf); // offset until displayed character
long calc_offset_act(uint pos, uint i, gap_buf &buf); // offset from i until byte pos
uint dchar2bytes(uint dx, uint from, gap_buf &buf); // how many bytes are dx displayed characters
