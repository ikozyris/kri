...
inline uint64_t pow2(uint64_t n) { return 1ul << n; }
inline unsigned log2(uint64_t n) { return 63u - __builtin_clzl(n); }
...
void resize(gap_buf &a, uint64_t size); // resize the buffer
void mv_curs(gap_buf &a, uint64_t pos); // move the cursor to position
void insert_c(gap_buf &a, uint64_t pos, char ch); // insert character at position
void insert_s(gap_buf &a, uint64_t pos, const char *str, uint64_t len); // insert string with given length at pos
void apnd_c(gap_buf &a, char ch); // append character
void apnd_s(gap_buf &a, const char *str, uint64_t size); // append string with given size
...
uint64_t data(gap_buf &src, uint64_t from, uint64_t to); // copy buffer with range to lnbuf
char at(gap_buf &src, uint64_t pos); // return character at position calculating the gap
uint64_t data2(gap_buf &src, uint64_t from, uint64_t to); // same as data(), different implementation
uint64_t shrink(gap_buf &a); // shrink buffer to just fit size (gap = 2 bytes)
